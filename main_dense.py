...
if __name__ == "__main__":
    ...
    mi_critic = Critic(input_dim=128*2, hidden_dim=512).to(device)
    lambda_mi = 5 #to-do: revert to 10 for possibly better performance
    ...
    print(f'Training detail: learning rate: 1e-4, weight decay: 5e-3, total epochs: {total_epoch}, batch size: {batch_size}, seed: {rand_seed}')
    ...
    for epoch in range(total_epoch_1):
        ...
        # print(f"Total Loss | Avg Loss: {sum(total_loss) / len(total_loss):.3f}")
    ...
    torch.save(model.state_dict(), f"./checkpoints/Dense_noscheduler_{timestamp}.pt")
...
# nohup python -u main_dense.py > ./log/Dense_noscheduler_$(date +%Y%m%d_%H%M%S).log 2>&1 &
