...
from utils import text_loss, fix_seed, sample_single_task_batch, collate_fn, SST2Dataset, get_test_loader_for_epoch, moe_balancing_loss, moe_balancing_loss_p_penalty, mutual_information_loss, Critic
...
if __name__ == "__main__":
    ...
    model = MoE_SemCom(num_tasks=2, embed_dim=412, task_dim=8, num_experts=8, num_encd_layer=4, transmit_dim=128).to(device)
    ...
    num_training_steps_1 = len(train_loader) * (total_epoch)
    ...
    num_training_steps_2 = len(train_loader) * (total_epoch)
    ...
    for epoch in range(total_epoch):
        ...
        # Save the model state after certain epochs 
        # save_every = 50  # Save every 50 epochs
        # if (epoch + 1) % save_every == 0:
        #     timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        #     torch.save(model.state_dict(), f"./checkpoints/MoE_epoch_{epoch+1}_{timestamp}.pt")
        #     # print(f"Model saved at epoch {epoch+1} with timestamp {timestamp}")
            
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    torch.save(model.state_dict(), f"./checkpoints/MoE_complete_{timestamp}.pt")
...
# nohup python -u main_moe.py > ./log/MoE_complete_$(date +%Y%m%d_%H%M%S).log 2>&1 &