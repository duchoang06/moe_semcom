...
class HetereoExpertFFN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_experts=4, top_k=2, tau=1.0, hard=False, size_distribution='uniform'):
        ...
        if size_distribution == 'uniform':
            ...
            self.experts = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(input_dim, hidden_dim + i * 128),
                    nn.ReLU(),
                    nn.Linear(hidden_dim + i * 128, input_dim)
                ) for i in range(num_experts)
            ])
            self.expert_sizes = torch.arange(hidden_dim, hidden_dim + num_experts *128, 128, dtype=torch.int32)
