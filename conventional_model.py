...
#     def __init__(self, H, G, max_iter=1e2):
#         super().__init__()
#         self.H = H
#         self.G = G
...
#     def forward(self, y, snr_arr=None):
#         # decoded_arr = []
#         # for i in y:
...
#         args_list = [(self.H, self.G, y[i], snr_arr[i], self.max_iter) for i in range(len(y))]
...
#         self.channel_decoder = LDPCDecoder(self.channel_encoder.H, self.channel_encoder.G, max_iter=1e2)
...
#     def forward(self, x, snr=12.0, fading='none', rician_k=3.0, modal=False):
...
#         rx_signal, snr_fading_arr = self.physical_channel(channel_encoded, snr=snr, fading='none', rician_k=rician_k, modal=modal)

#         rx_signal = np.array(rx_signal, dtype=np.float64)

#         channel_decoded = self.channel_decoder(rx_signal, snr_arr=snr_fading_arr)
